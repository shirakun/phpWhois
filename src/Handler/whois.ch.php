...
class ch_handler
{
    ...
    public function parse($data_str, $query)
    {
        ...
        if (!empty($r['regrinfo']['domain']['name'])) {
            $r['regrinfo'] = Parser::get_contacts($r['regrinfo'], $trans);

            $r['regrinfo']['domain']['name'] = $r['regrinfo']['domain']['name'][0];

            if (isset($r['regrinfo']['domain']['changed'][0])) {
                $r['regrinfo']['domain']['changed'] = Parser::get_date($r['regrinfo']['domain']['changed'][0], 'dmy');
            }

            if (isset($r['regrinfo']['domain']['created'][0])) {
                $r['regrinfo']['domain']['created'] = Parser::get_date($r['regrinfo']['domain']['created'][0], 'dmy');
            }
