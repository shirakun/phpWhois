...
class krnic_handler
{
    public function parse($data_str, $query)
    {
        ...
        $b = Parser::get_blocks($data_str, $blocks);
        ...
        if (isset($b['owner1'])) {
            $r['owner'] = Parser::generic_parser_b($b['owner1'], $items, 'Ymd', false);
        } else
        if (isset($b['owner2'])) {
            $r['owner'] = Parser::generic_parser_b($b['owner2'], $items, 'Ymd', false);
        }

        if (isset($b['admin2'])) {
            $r['admin'] = Parser::generic_parser_b($b['admin2'], $items, 'Ymd', false);
        } else
        if (isset($b['admin3'])) {
            $r['admin'] = Parser::generic_parser_b($b['admin3'], $items, 'Ymd', false);
        }

        if (isset($b['tech1'])) {
            $r['tech'] = Parser::generic_parser_b($b['tech1'], $items, 'Ymd', false);
        } else
        if (isset($b['tech2'])) {
            $r['tech'] = Parser::generic_parser_b($b['tech2'], $items, 'Ymd', false);
        } else
        if (isset($b['tech3'])) {
            $r['tech'] = Parser::generic_parser_b($b['tech3'], $items, 'Ymd', false);
        }

        if (isset($b['abuse'])) {
            $r['abuse'] = Parser::generic_parser_b($b['abuse'], $items, 'Ymd', false);
        }

        $r = Util::format_dates($r, 'Ymd');
