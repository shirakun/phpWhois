...
class jp_handler extends WhoisClient
{
    public function parse($data_str, $query)
    {
        ...
        $r['regrinfo'] = Parser::generic_parser_b($data_str['rawdata'], $items, 'ymd');
        ...
        if (!empty($r['regrinfo']['admin']['handle'])) {
            ...
            $r['regrinfo']['admin'] = Parser::generic_parser_b($rwdata, $items, 'ymd', false);
        ...
        if (!empty($r['regrinfo']['tech']['handle'])) {
            if (!empty($r['regrinfo']['admin']['handle']) &&
                ...
                $r['rawdata'][]        = '';
                $r['rawdata']          = array_merge($r['rawdata'], $rwdata);
                $r['regrinfo']['tech'] = Parser::generic_parser_b($rwdata, $items, 'ymd', false);
                $r                     = $this->set_whois_info($r);
            }
        }
