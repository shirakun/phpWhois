...
class zanet_handler
{
    public function parse($data_str, $query)
    {
        ...
        $r['regrinfo'] = Parser::get_blocks($rawdata, $items);

        if (isset($r['regrinfo']['registered'])) {
            $r['regrinfo']['registered'] = 'no';
        } else {
            if (isset($r['regrinfo']['admin'])) {
                $r['regrinfo']['admin'] = Parser::get_contact($r['regrinfo']['admin']);
            }

            if (isset($r['regrinfo']['tech'])) {
                $r['regrinfo']['tech'] = Parser::get_contact($r['regrinfo']['tech']);
            }
