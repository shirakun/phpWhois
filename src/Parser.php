...
class Parser
{
    public static function generic_parser_a($rawdata, $translate, $contacts, $main = 'domain', $dateformat = 'dmy')
    {
        ...
        self::format_dates($ret, $dateformat);
        return $ret;
    }

    //-------------------------------------------------------------------------

    public static function generic_parser_a_blocks($rawdata, $translate, &$disclaimer)
    {
        $r        = array();
        $newblock = false;
        $hasdata  = false;
    ...
    public static function generic_parser_b($rawdata, $items = false, $dateformat = 'mdy', $hasreg = true, $scanall = false)
    {
        if (!$items) {
            $items = array(
    ...
    public static function assign_recursive($array, $parts, $value)
    {
        ...
        if (count($parts) == 0) {
            ...
            $array[$key] = self::assign_recursive($array[$key], $parts, $value);
        }
    ...
    public static function assign($array, $vdef, $value)
    {
        return self::assign_recursive($array, explode('.', $vdef), $value);
    }

    //-------------------------------------------------------------------------

    public static function get_blocks($rawdata, $items, $partial_match = false, $def_block = false)
    {
            ...
            foreach ($items as $field => $match) {
                ...
                if ($field != '' && $pos !== false) {
                    ...
                    if ($last == ':' || $last == '-' || $last == ']') {
                        ...
                        $r = self::assign($r, strtok($field, '#'), trim(substr($val, $pos + strlen($match))));
                    }
    ...
    public static function easy_parser($data_str, $items, $date_format, $translate = false,
        $has_org = false, $partial_match = false,
        $def_block = false) {
        $r = self::get_blocks($data_str, $items, $partial_match, $def_block);
        $r = self::get_contacts($r, $translate, $has_org);
        self::format_dates($r, $date_format);
        return $r;
    }

    //-------------------------------------------------------------------------

    public static function get_contacts($array, $extra_items = '', $has_org = false)
    {
    ...
    public static function get_contact($array, $extra_items = '', $has_org = false)
    {
    ...
    public static function format_dates(&$res, $format = 'mdy')
    {
        if (!is_array($res)) {
            return $res;
        }
